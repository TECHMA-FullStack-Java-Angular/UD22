...
public class ListController implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == view.insert) {
			if (table == DataBase.cientificos)
				new EditCientificos_Controller(this);
			else if (table == DataBase.proyectos)
				new EditProyectos_Controller(this);
			else if (table == DataBase.asignaciones)
				new EditAsignaciones_Controller(this);
		}
		else if (e.getSource() == view.update) {
			String[] row_values = getRowValues();
			if (row_values != null) {
				if (table == DataBase.cientificos)
					new EditCientificos_Controller(this, row_values);
				else if (table == DataBase.proyectos)
					new EditProyectos_Controller(this, row_values);
				else if (table == DataBase.asignaciones)
					new EditAsignaciones_Controller(this, row_values);
			}
		}
